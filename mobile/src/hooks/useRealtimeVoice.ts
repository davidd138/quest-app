...
export function useRealtimeVoice(character: { name: string; role: string; personality?: string }) {
  ...
  const buildSystemPrompt = useCallback(() => {
    return [
      ...
      '- Respond naturally as this character would in the quest scenario.',
      '- Be engaging and provide clues or challenges appropriate to the quest stage.',
      '- Keep responses concise, as in a real conversation.',
      '- React to what the player says and guide them through the quest.',
    ].filter(Boolean).join('\n');
  }, [character]);
